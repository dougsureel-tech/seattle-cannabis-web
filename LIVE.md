...
|---|---|---|---|
| 2026-05-19 | v29.025 | 717e9aa | 🎨 /menu-preview cycle-4 ROUND 2 — three more polish items + one regression fix from round 1. (1) **Chip-collision regression fix from v29.005**: strain chip was conditionally `top-2.5 left-2.5` when no deal — collided with my v29.005 DOH chip in the same corner on DOH+strain cards. Strain chip now ALWAYS `bottom-2.5 right-2.5` (informational; top corners reserved for higher-priority deal/DOH/NEW/cart chips). Bottom edge reads as symmetric weight-left / strain-right. (2) **Empty-state next-steps overhaul**: bolder primary message, context sub-line, pill-button Clear/Reset CTAs (was tiny underline link), NEW category-pivot row showing top-4 in-stock categories as one-tap recovery buttons (resets all filters + jumps to that category). Search-miss path gets distinct copy with phone-the-store fallback. (3) **Hero contrast bump**: `text-green-300/70` + `text-green-200/70` → `text-green-100` + `text-green-200/90` — clears WCAG AA on slate-900 hero. Pre-commit Explore CLEAN. typecheck CLEAN. 400/400 tests passing. Verify `v29.025 / sha=717e9aa / ok=true`. |
| 2026-05-19 | v29.005 | ce05d2b | 🎨 /menu-preview cycle-4 polish — round 1 of 2 per Doug "looks like dookei" feedback. 3 shipped: (a) `ProductImage` brand-logo fallback — when product has no `image_url` but `brand` slug matches a file at `/public/brand-logos/<slug>.png` (60+ brand logos already shipped), render the brand logo at 96px contained INSTEAD of the generic category icon. `onError` gracefully drops back to category icon for brands without a logo file — no per-brand allowlist, no broken-image risk. Brand pill suppressed when logo renders successfully. (b) DOH chip placement — `top-12` when deal-pill present (stacked), `top-2.5` solo when no deal — replaces prior collision with the deal `%-OFF` chip top-left. Swapped 🟣 emoji for inline `<svg>` medical cross — reads as clinical regulatory chip, not consumer marketing. `<TODO swap inline SVG for the official WA-DOH logo>` comment marks Doug-action site (logo asset didn't survive compaction; Doug to re-drop). (c) Responsive grid gap — `gap-3 sm:gap-4` (was flat `gap-4`). Round 2 backlog at `/CODE/PROJECT_BOARD.md` SCC Todo: filter rail mobile overflow + loading skeletons + empty-state copy + hero contrast. Pre-commit Explore review (caught 0 real issues; flagged-but-incorrect stale-closure warning). typecheck CLEAN. 400/400 tests passing. Verify `v29.005 / sha=ce05d2b / ok=true`. |
