...
|---|---|---|---|
| 2026-06-12 | **v34.686** | dab162b | 🛡️ WSLCB efficacy-FRAME sister-port — 3 frame regexes (good/great/perfect-for · knocks-out · calms-you/your) added to `scrubWslcbClaims` in `lib/strain-product-json-ld.ts` (frame-only, won't mangle "Calm Tincture"); all 3 copies (inv-App + glw + scc) now in sync. Coupled copy-fix: dropped redundant `.` after `${STORE.name}` (="Co.") in `find-your-strain/result` description (pre-existing double-period gate offender). Verify-via-curl `v34.686 / sha=dab162b / ok=true`. 1955/1955 tests. ⚠️ Follow-up: sister-port the GLW `check-csp-report-only` enforce-mode gate fix here on next push. |
| 2026-06-10 | **v34.645** | da35b02 | 🔒 Per-PHONE OTP rate-cap on `/api/rewards/request-code` (toll-fraud / SMS-bomb defense). Independent of the existing per-IP cap so a distributed / IP-rotating attacker can't flood ONE victim's number with paid Twilio SMS. DB COUNT on `loyalty_otp_codes.phone` over last hour, `MAX_PER_PHONE_PER_HOUR=5`, same generic 429 as the IP path (no enumeration oracle — counts "requested N codes recently", true for any number). Verify-via-curl `v34.645 / sha=da35b02 / ok=true`. tsc clean. Pre-push Explore CLEAN. |
| 2026-06-09 | **v34.635** | fb02c85 | 🛒 Native-menu `?category=` deep-link resolver + true Vapes family group (sister glw v43.135). `resolveCategoryParam()` maps homepage tile slugs to in-stock canonical buckets (never empty grid); `CATEGORY_GROUPS.Vapes` spans Cartridges+Disposables+Pods via `categoryMatches()` (Doug 2026-06-09 "all in one = disposable"). Dormant until `NATIVE_MENU_LIVE` flips. Verify-via-curl `v34.635 / sha=fb02c85 / ok=true`. tsc clean. Pre-push Explore CLEAN. |
| 2026-05-30 | **v34.045** | fb0f73c | 🛍️ **PDP preview surface `/menu/preview/[id]` — Phase 0 of Product UX Redesign (sister glw v42.745).** New Curaleaf-hierarchy product detail page at noindex preview URL — hero image OR brand-fallback specimen, brand small-caps (indigo-700), oversized THC% (text-5xl indigo-800), effect chips via keyword match on product `notes`/`effects` (WAC 314-55-155 preference-context register), terpene-dominance pills, price + "Out the door: $X" (1.4755× shelf for Seattle 37%+10.55%), full-width Add-to-cart CTA (indigo-700) wiring to existing `gl_cart` localStorage + `/menu-preview?cart=open` drawer-open mechanism, verified-purchaser reviews section (hidden when count=0 per UX brief), tenure footer ("Since 2010 · Pre-I-502 medical heritage"). 7 NEW pure helpers at `lib/pdp-helpers.ts` (44 pin tests). NEW `getPreviewProductBundle` DB fn with brand-recent-sales gate + floor-only two-bucket discipline. Sister-port fix-forward: round2-coverage.test.ts + pacific-date-coverage.test.ts add `.claude/` to walker exclusion (byte-shape parity with glw — closes pre-existing drift). **Re-bump from v34.025:** sister-session v34.005 commit accidentally bundled my v34.025 changelog text without the code; v34.045 lands the actual code. Click-test: visit `/menu/preview/<product-id>` directly. |
| 2026-05-21 | v29.975 | (pending) | 🧪 strain-families refactor + pin tests (sister glw v38.645). 1-line import refactor `@/lib/strains` → `./strains.ts` (matches established `./sibling.ts` pattern across 9+ other lib files). 23 new pin tests cover 10-family shape + anchor referential integrity vs STRAINS + WAC voice + helper contracts. Tests 603 → 626. Pre-commit Explore CLEAN. |
| 2026-05-21 | v29.955 | a070e16 | 🖼️ 5 new brand logos sourced from BRAND-OWN sites (sister glw v38.625, Doug 2026-05-21 backfill). Added phat-panda · plaid-jacket · airo · flipside · harmony-farms. Coverage 29 → 34. Per parallel `claude-brand-logo-backfill-2026-05-20` session. |
| 2026-05-21 | v29.935 | 6abda84 | 🧪 Batch 4 pin tests — `lib/learn-hub.ts` SoT (sister glw v38.585). 22 new tests pinning 8-topic shape + WAC voice (no exclamation in title/desc/faqs) + relatedTopics referential integrity + SERP description length + helper contracts. strain-families.ts deferred to v29.975 (refactor + pin tests). Tests 581 → 603. Pre-commit Explore CLEAN. | — `lib/learn-hub.ts` SoT (sister glw v38.585). 22 new tests pinning 8-topic shape + WAC voice (no exclamation in title/desc/faqs) + relatedTopics referential integrity + SERP description length + helper contracts. strain-families.ts deferred (`@/lib/strains` value-import doesn't resolve under Node `--experimental-strip-types`). Tests 581 → 603. Pre-commit Explore CLEAN. |
| 2026-05-21 | v29.915 | e224028 | 🧪 Batch 3 pin tests — `lib/client-ip.ts` + `lib/store-time.ts` + `lib/time-constants.ts` (sister glw v38.565). 45 new tests (17+14+14). client-ip pins truthy-check-vs-?? distinction (whitespace x-forwarded-for must fall through). store-time pins Pacific-tz correctness via 06:00-UTC → prior-day test. time-constants pins composition (DAY_MS === 24*HOUR_MS). Tests 536 → 581. Pre-commit Explore CLEAN. Verify-via-curl `v29.915 / sha=e224028 / ok=true`. |
| 2026-05-21 | v29.895 | 9b8ce31 | 🧪 Batch 2 pin tests — `lib/brand-logos-available.ts` + `lib/medical-pricing.ts` + `lib/online-pricing.ts` (sister glw v38.545). 57 new tests (18+14+25). medical-pricing tests are stack-portable via CANNABIS_TAX_SAVINGS_PCT round-trip (handles SCC 10.55% vs GLW 8.8% sales-tax divergence). Tests 479 → 536. Pre-commit Explore CLEAN. Verify-via-curl `v29.895 / sha=9b8ce31 / ok=true`. |
| 2026-05-21 | v29.875 | 188a20d | 🧪 NEW pin tests for `lib/safe-redirect.ts` + `lib/banned-logo-url.ts` (sister glw v38.525). 62 new tests (25 + 37), 0 prod behavior change. Tests 417 → 479. Pre-commit Explore CLEAN. Verify-via-curl `v29.875 / sha=188a20d / ok=true`. |
| 2026-05-19 | v28.965 | 068fe39 | 🩹 /apply UX hardening — Tyler-referred Wenatchee applicant couldn't upload resume (no specific error). 2 adds: NEW collapsed `<details>` "Not sure how to make a PDF?" under the resume drop zone (Word/Pages/Google Docs/iPhone Notes scan recipes; indigo brand tokens) · NEW "Still needed: …" hint under disabled Submit button listing exactly which required fields are empty (was: button greyed silently with no signal). `<details>` placed OUTSIDE the `<label>` wrapper so the summary tap doesn't trigger the hidden file input via implicit label-input association. Sister glw v37.605 same-push. Verify `v28.965 / sha=068fe39 / ok=true`. typecheck CLEAN. Pre-commit Explore CLEAN. |
